...
uint32_t Clipboard::Private::onClientFormatList(const CLIPRDR_FORMAT_LIST *formatList)
{
    for (uint32_t i = 0; i < formatList->numFormats; ++i) {
        ...
        case CF_UNICODETEXT:
        case CF_OEMTEXT: {
            CLIPRDR_FORMAT_DATA_REQUEST formatDataRequest{.msgType = CB_FORMAT_DATA_REQUEST, .msgFlags = 0, .dataLen = 4, .requestedFormatId = CF_UNICODETEXT};
...
uint32_t Clipboard::Private::onClientFormatDataRequest(const CLIPRDR_FORMAT_DATA_REQUEST *formatDataRequest)
{
    ...
    CLIPRDR_FORMAT_DATA_RESPONSE response{.msgType = CB_FORMAT_DATA_RESPONSE,
                                          .msgFlags = CB_RESPONSE_OK,
                                          .dataLen = uint32_t(data.length()) * 2,
...
uint32_t Clipboard::Private::onClientFormatDataResponse(const CLIPRDR_FORMAT_DATA_RESPONSE *formatDataResponse)
{
    ...
    clientData->setText(QString::fromUtf16(reinterpret_cast<const char16_t *>(formatDataResponse->requestedFormatData), formatDataResponse->dataLen / 2 - 1));
